...
export const articles: Article[] = [
  ...
  },
  {
    ...
    summary:
      'Google Cloud Spanner 官方 Codex 插件：marketplace 加 GoogleCloudPlatform/data-agent-kit，再 plugin add spanner@data-agent-kit，项目用 SPANNER_PROJECT 不要抄 spanner@claude-plugins-official。Codex 要 v0.117.0+。MCP 表名是 spanner，不要 mcp login。',
  },
  {
    ...
    summary:
      'GCP 这篇讲用 MCP Toolbox 连 Spanner。主推 Gemini CLI 的 gemini extensions install，其它 IDE 才抄本机 toolbox 二进制和 --prebuilt spanner。没有 Codex 的 plugin add。坐标仍是 SPANNER_PROJECT / INSTANCE / DATABASE。',
  },
  {
  ...
  },
  {
    ...
    summary:
      'Google Cloud Cloud SQL for SQL Server 官方 Codex 插件：marketplace 加 GoogleCloudPlatform/data-agent-kit，再 plugin add cloud-sql-sqlserver@data-agent-kit，项目用 CLOUD_SQL_MSSQL_PROJECT 不要写成 plugin install。Codex 要 v0.117.0+。MCP 表名是 cloud-sql-mssql，用户密码必填，不要 mcp login。',
  },
  {
    ...
    summary:
      'GCP 这篇讲用 MCP Toolbox 连 Cloud SQL for SQL Server。Codex 节把动词写成 plugin install；插件仓 README 是 plugin add。Cursor 示例是 mcpServers.cloud-sql-sqlserver 加本机 toolbox 二进制和 --prebuilt cloud-sql-mssql。坐标是 CLOUD_SQL_MSSQL_*，不是 CLOUD_SQL_SQLSERVER_*。',
  },
  {
    ...
    summary:
      'Cloud SQL 远程 MCP 是 https://sqladmin.googleapis.com/mcp，开 API 即启用。这不是 Codex 插件那条本机 toolbox stdio。客户端用 Google 凭证，不是 mcp login 插件表。不要和 cloud-sql-sqlserver@data-agent-kit 抄成一条。',
  },
  {
    ...
    summary:
      'Google Cloud Firestore Native 官方 Codex 插件：marketplace 加 GoogleCloudPlatform/data-agent-kit，再 plugin add firestore-native@data-agent-kit，项目用 FIRESTORE_PROJECT 不要写成 plugin install。Codex 要 v0.117.0+。MCP 表名是 firestore，不要写成 plugin add firestore@，也不要 mcp login。',
  },
  {
    ...
    summary:
      'GCP 这篇讲用 MCP Toolbox 连 Firestore Native。页面只给了 Gemini / Cursor / Claude 的 toolbox 二进制，没有 Codex 专节。Cursor 示例是 mcpServers.firestore 加本机 toolbox 二进制和 --prebuilt firestore。坐标是 FIRESTORE_PROJECT，可选 FIRESTORE_DATABASE。',
  },
  {
